...
from models import KnowledgeSource, TrashBinItem, UserActivity, UserFeedback, ConflictStat
...
def generate_weekly_report() -> Dict[str, Any]:
    ...
    logger.info(f"Weekly report generated: {len(report['worked_well'])} positives, "
...
def _gather_positive_metrics(start_date: datetime, end_date: datetime) -> List[str]:
    ...
    try:
        ...
        new_sources = session.query(KnowledgeSource).filter(
        ...
        conflicts_resolved = session.query(ConflictStat).filter(
            ConflictStat.last_seen >= start_date
        ).count()
        ...
        # Check curation runs
        curation_runs = session.query(UserActivity).filter(
            UserActivity.event_type == "curation_run",
        ...
        trash_compacted = session.query(TrashBinItem).filter(
            TrashBinItem.created_at >= start_date,
            TrashBinItem.action_taken == "compacted"
        ).count()
        ...
        high_cred_sources = session.query(KnowledgeSource).join(
            SourceCredibility, KnowledgeSource.credibility_id == SourceCredibility.id
        ).filter(
            KnowledgeSource.created_at >= start_date,
            SourceCredibility.overall_score >= 0.8
        ).count()
...
def _gather_issues(start_date: datetime, end_date: datetime) -> List[str]:
    ...
    try:
        ...
        failed_curations = session.query(UserActivity).filter(
        ...
        trash_backlog = session.query(TrashBinItem).filter(
        ...
        low_cred_sources = session.query(SourceCredibility).filter(
            SourceCredibility.overall_score < 0.4,
            SourceCredibility.created_at >= start_date
        ).count()
        ...
        untagged = session.query(KnowledgeSource).filter(
            KnowledgeSource.tags.is_(None),
            KnowledgeSource.created_at >= start_date
        ).count()
        ...
        missing_version = session.query(KnowledgeSource).filter(
            KnowledgeSource.game_version.is_(None),
            KnowledgeSource.created_at >= start_date
        ).count()
        ...
        orphaned = session.query(KnowledgeSource).filter(
            KnowledgeSource.category.is_(None),
            KnowledgeSource.created_at >= start_date
        ).count()
...
def _generate_suggestions(start_date: datetime, end_date: datetime) -> List[str]:
    ...
    try:
        ...
        top_accessed = session.query(KnowledgeSource).filter(
            KnowledgeSource.last_accessed >= start_date
        ).order_by(KnowledgeSource.last_accessed.desc()).limit(10).all()
        
        if top_accessed:
            # Suggest caching for frequently accessed
            suggestions.append(f"Pre-compute PDF cache for top {len(top_accessed)} queries (reduces generation time)")
        
        # Analyze search patterns
        search_activities = session.query(UserActivity).filter(
        ...
        for (game,) in games:
            version_coverage = session.query(KnowledgeSource).filter(
                KnowledgeSource.game == game,
                KnowledgeSource.game_version.isnot(None)
            ).count()
            
            total_for_game = session.query(KnowledgeSource).filter(
                KnowledgeSource.game == game
            ).count()
            ...
            if total_for_game > 0:
                ...
                if coverage_pct < 70:
                    suggestions.append(f"Improve {game} version tagging (currently {coverage_pct:.0f}% coverage)")
        
        # Check research pipeline efficiency
        research_activities = session.query(UserActivity).filter(
            UserActivity.event_type == "research_run",
            UserActivity.created_at >= start_date
        ).count()
...
def _gather_new_knowledge(start_date: datetime, end_date: datetime) -> List[str]:
    ...
    try:
        ...
        conflict_rules = session.query(KnowledgeSource).filter(
            KnowledgeSource.created_at >= start_date,
            KnowledgeSource.conflicts_with.isnot(None)
        ).count()
        ...
        verified = session.query(KnowledgeSource).filter(
            KnowledgeSource.created_at >= start_date,
            KnowledgeSource.credibility_id.isnot(None)
        ).join(SourceCredibility).filter(
            SourceCredibility.overall_score >= 0.7
        ).count()
...
def _generate_questions(report: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        if len(report.get("needs_improvement", [])) > 3:
            questions.append("Budget alert: Multiple issues detected. Approve additional resources for fixes?")
        
        # Feature requests
        questions.append("Should we prioritize Starfield modding support? (Check user feedback trends)")
        ...
        critical_issues = [i for i in report.get("needs_improvement", []) if "failed" in i.lower() or "backlog" in i.lower()]
        if critical_issues:
            questions.append(f"Action required: {len(critical_issues)} critical issues need attention")
