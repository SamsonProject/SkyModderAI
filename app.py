...
OPENCLAW_REQUIRE_SAME_IP = os.environ.get('OPENCLAW_REQUIRE_SAME_IP', '1').lower() in ('1', 'true', 'yes')
...
OPENCLAW_DENY_SEGMENTS = frozenset({
    ...
    'windows', 'system32', 'syswow64', 'drivers', 'registry',
    'bios', 'efi', 'boot', 'proc', 'dev', 'etc', 'usr', 'var',
...
def get_user_tier(email):
    ...
    try:
        db = get_db()
        row = db.execute(
            'SELECT tier FROM users WHERE email = ?', (email.lower(),)
        ).fetchone()
...
def get_user_specs(email):
    ...
    try:
        row = get_db().execute(
            'SELECT specs_json FROM user_specs WHERE user_email = ?',
            (email.lower(),)
        ).fetchone()
...
def ensure_user_unverified(email, password=None):
    ...
    try:
        ...
        if password and db.total_changes == 0:
            db.execute('UPDATE users SET password_hash = ?, last_updated = CURRENT_TIMESTAMP WHERE email = ?', (pwhash, email))
        db.commit()
    except Exception as e:
...
def is_user_verified(email):
    ...
    try:
        db = get_db()
        row = db.execute('SELECT email_verified FROM users WHERE email = ?', (email.lower(),)).fetchone()
...
def session_get(token):
    ...
    try:
        ...
        if not row or (row['expires_at'] or 0) < now_ts:
            return None
        db.execute('UPDATE user_sessions SET last_seen = CURRENT_TIMESTAMP WHERE token = ?', (token,))
        db.commit()
        return row
    except Exception as e:
...
def session_revoke_all_other(user_email, keep_token):
    ...
    try:
        db = get_db()
        db.execute('DELETE FROM user_sessions WHERE user_email = ? AND token != ?', (user_email.lower(), keep_token))
        db.commit()
    except Exception as e:
...
def session_list(user_email, current_token=None):
    ...
    try:
        ...
        for r in rows:
            ...
            if not display_id:
                display_id = secrets.token_urlsafe(8)
                db.execute('UPDATE user_sessions SET display_id = ? WHERE token = ?', (display_id, r['token']))
                db.commit()
            out.append({
...
def session_revoke_by_display_id(user_email, display_id):
    ...
    try:
        db = get_db()
        db.execute('DELETE FROM user_sessions WHERE user_email = ? AND display_id = ?', (user_email.lower(), display_id))
        db.commit()
...
def api_key_create(user_email, label=None):
    ...
    try:
        ...
        db.commit()
        row = db.execute('SELECT id, created_at FROM api_keys WHERE key_hash = ?', (key_hash,)).fetchone()
...
def api_key_lookup(raw_key):
    ...
    try:
        row = get_db().execute('SELECT user_email FROM api_keys WHERE key_hash = ?', (key_hash,)).fetchone()
...
def api_key_revoke(user_email, key_id):
    ...
    try:
        db = get_db()
        db.execute('DELETE FROM api_keys WHERE id = ? AND user_email = ?', (int(key_id), user_email.lower()))
        db.commit()
        return db.total_changes > 0
    except Exception as e:
...
def login_submit():
    ...
    if not row or not row.get('password_hash'):
        return jsonify({'error': 'No account with that email, or account has no password set. Sign up first.'}), 401
    ...
    token, max_age = session_create(email, remember_me=remember_me, user_agent=request.headers.get('User-Agent'))
    resp = jsonify({'success': True, 'redirect': next_url})
    if token:
        resp.set_cookie(
            SESSION_COOKIE_NAME, token,
            max_age=max_age, httponly=True, secure=app.config['SESSION_COOKIE_SECURE'],
...
def verify_email():
    ...
    sess_token, max_age = session_create(email, remember_me=True, user_agent=request.headers.get('User-Agent'))
    resp = redirect(url_for('verified'))
    if sess_token:
        resp.set_cookie(
            SESSION_COOKIE_NAME, sess_token,
            max_age=max_age, httponly=True, secure=app.config['SESSION_COOKIE_SECURE'],
            samesite=app.config['SESSION_COOKIE_SAMESITE']
        )
    return resp
...
def profile():
    ...
    return render_template('profile.html', email=user_email, tier=user_tier, tier_display=tier_display, sessions=sessions, api_keys=api_keys, google_oauth_enabled=GOOGLE_OAUTH_ENABLED, customer_id=customer_id, payments_enabled=PAYMENTS_ENABLED, specs=specs, links=links)
...
def api_developer_keys_create():
    ...
    raw, key_id, prefix, label_out, created_at = api_key_create(session['user_email'], label=label or None)
    if not raw:
...
_LINK_PREVIEW_ALLOWED_DOMAINS = frozenset([
    ...
    'tes5edit.github.io', 'loot.github.io', 'github.com', 'www.github.com',
    'youtube.com', 'www.youtube.com', 'youtu.be',
    'wabbajack.org', 'www.wabbajack.org',
    'reddit.com', 'www.reddit.com', 'old.reddit.com',
    'silverlock.org', 'skse.silverlock.org', 'f4se.silverlock.org',
])
...
def _is_preview_domain_allowed(netloc: str) -> bool:
    ...
    return (
        netloc in _LINK_PREVIEW_ALLOWED_DOMAINS or
        netloc_clean in _LINK_PREVIEW_ALLOWED_DOMAINS or
        any(netloc == d or netloc.endswith('.' + d) for d in _LINK_PREVIEW_ALLOWED_DOMAINS)
    )
...
def _extract_meta_preview(doc: str, resolved_url: str) -> dict:
    ...
    def _og(s):
        ...
        if not m:
            m = re.search(r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+property=["\']og:' + s + r'["\']', doc, re.I)
    ...
    return {'url': resolved_url, 'title': title or '', 'description': (desc or '')[:300], 'image': image or ''}
...
def api_link_reader():
    ...
    try:
        if _is_internal_preview_url(url):
            ...
            r = requests.get(url, timeout=6, headers={'User-Agent': 'SkyModderAI/1.0 (link reader)'})
            r.raise_for_status()
            doc = r.text
...
_COMMUNITY_BLOCKLIST = frozenset([
    'kill yourself', 'kys', 'die ', ' go die', 'unalive', 'harass', 'dox ',
    'nazi', 'hitler', 'rape', 'pedo', 'pedophile', 'slur', 'retard', 'faggot',
    'nigger', 'nigga', 'chink', 'spic', 'kike', 'gook', 'tranny',
    'human trafficking', 'drug trafficking', 'buy cocaine', 'sell meth',
    'hitman for hire', 'hire a hitman',
])
...
COMMUNITY_TAGS = ['general', 'tip', 'help', 'celebration', 'skyrim', 'fallout', 'starfield', 'tool', 'dev']
...
def api_community_posts_list():
    ...
    try:
        ...
        for r in rows:
            ...
            vote_sum = db.execute('SELECT COALESCE(SUM(vote), 0) FROM community_votes WHERE post_id = ?', (r['id'],)).fetchone()[0] or 0
            my_vote = 0
            if session.get('user_email'):
                v = db.execute('SELECT vote FROM community_votes WHERE post_id = ? AND user_email = ?', (r['id'], session['user_email'].lower())).fetchone()
                if v:
                    my_vote = v['vote']
            posts.append({
                'id': r['id'],
                ...
                'created_at': r['created_at'],
                'is_pro': has_paid_access(tier),
                'replies': [{'id': rr['id'], 'user': _redact_email(rr['user_email']), 'content': rr['content'], 'created_at': rr['created_at'], 'is_pro': has_paid_access(get_user_tier(rr['user_email']))} for rr in reply_rows],
                'votes': vote_sum,
                'my_vote': my_vote,
            })
...
def api_community_posts_create():
    ...
    if not _community_content_ok(content):
        return jsonify({'error': 'Post must be 3–2000 characters and follow community guidelines. See the guidelines in the Community tab.'}), 400
    try:
        db = get_db()
        db.execute(
        ...
        db.commit()
        row = db.execute('SELECT id, created_at FROM community_posts WHERE id = last_insert_rowid()').fetchone()
        track_activity('community_post_create', {'post_id': row['id'], 'tag': tag}, session.get('user_email'))
...
def api_community_reply(post_id):
    ...
    content = (data.get('content') or '').strip()
    if not _community_content_ok(content):
        return jsonify({'error': 'Reply must be 3–2000 characters and follow community guidelines.'}), 400
    try:
        db = get_db()
        exists = db.execute('SELECT 1 FROM community_posts WHERE id = ? AND moderated = 0', (post_id,)).fetchone()
        if not exists:
        ...
        db.commit()
        row = db.execute('SELECT id, created_at FROM community_replies WHERE id = last_insert_rowid()').fetchone()
        track_activity('community_reply_create', {'post_id': post_id, 'reply_id': row['id']}, session.get('user_email'))
        return jsonify({'success': True, 'id': row['id'], 'created_at': row['created_at']})
    except Exception as e:
...
def api_community_vote(post_id):
    ...
    try:
        ...
        db.execute('DELETE FROM community_votes WHERE post_id = ? AND user_email = ?', (post_id, email))
        if vote != 0:
            db.execute('INSERT INTO community_votes (post_id, user_email, vote) VALUES (?, ?, ?)', (post_id, email, vote))
        db.commit()
        total = db.execute('SELECT COALESCE(SUM(vote), 0) FROM community_votes WHERE post_id = ?', (post_id,)).fetchone()[0] or 0
        track_activity('community_vote', {'post_id': post_id, 'vote': vote, 'votes_total': total}, session.get('user_email'))
...
def list_masterlist_versions(game_id):
    ...
    return jsonify({'game': game_id, 'versions': versions, 'latest': LOOTParser.LATEST_VERSIONS.get(game_id, '0.26')})
...
def api_recommendations():
    ...
    try:
        p = get_parser(game)
        if for_ai:
            payload = get_recommendations_for_ai(p, mod_list, game, nexus_slug, limit=limit, top_picks_per_category=2)
            out = {'recommendations': payload['recommendations'], 'top_picks': payload['top_picks'], 'game': game}
        else:
            recs = get_recommendations(p, mod_list, game, nexus_slug, limit=limit, include_category=True)
...
def ai_context():
    ...
    if request.method == 'GET':
        ...
        ctx = build_knowledge_context(game_id=game, conflicts=[], mod_list=[], specs=None, user_query=user_query)
        return jsonify(ctx)
    data = request.get_json() or {}
    game = (data.get('game') or DEFAULT_GAME).lower()
...
def search_solutions():
    ...
    user_tier = get_user_tier(user_email) if user_email else 'free'
    if not has_paid_access(user_tier):
        return jsonify({'solutions': [], 'error': 'Paid subscription required for web solution search'}), 403
    try:
...
def resolve_conflict():
    ...
    conflict_type = (request.args.get('type') or request.args.get('conflict_type') or '').strip().lower()
    game = (request.args.get('game') or DEFAULT_GAME).lower()
...
def analyze_mods():
    ...
    try:
        ...
        if not mods:
            return api_error('Could not parse mod list. Add one or more mods in a supported format (e.g. *ModName.esp per line).', 400)
        ...
        def _build_conflict_links(c, slug):
            ...
            if c.affected_mod:
                links.append({'title': 'Nexus: ' + (c.affected_mod[:30] + '…' if len(c.affected_mod) > 30 else c.affected_mod), 'url': base + quote(c.affected_mod)})
            related = getattr(c, 'related_mod', None)
            if related and related != (c.affected_mod or ''):
                links.append({'title': 'Nexus: ' + (related[:30] + '…' if len(related) > 30 else related), 'url': base + quote(related)})
        ...
        payload = {
            ...
            'report': detector.format_report() + (format_system_impact_report(system_impact) if system_impact else ''),
            'summary': {
            ...
            'ai_context': (
                detector.format_report_for_ai(game_name=game_name, nexus_slug=nexus_slug, specs=specs)
...
def api_v1_analyze():
    ...
    if not user_email:
        return api_error('Invalid or missing API key. Use Authorization: Bearer <key> or X-API-Key header.', 401)
    try:
        data = request.get_json() or {}
...
def payment_success():
    ...
    try:
        ...
        if checkout_session.payment_status == 'paid':
            ...
            sess_token, max_age = session_create(email, remember_me=True, user_agent=request.headers.get('User-Agent'))
            resp = make_response(render_template('success.html', email=email, tier=tier_label(purchased_tier)))
            if sess_token:
                resp.set_cookie(
                    SESSION_COOKIE_NAME, sess_token,
                    max_age=max_age, httponly=True, secure=app.config['SESSION_COOKIE_SECURE'],
...
_PROBLEM_KEYWORDS = frozenset([
    'ctd', 'crash', 'fix', 'broken', 'not working', 'help', 'issue', 'problem',
    'infinite loading', 'ils', 'purple', 'texture', 't-pose', 'tpose',
    'error', 'stuck', 'freeze', 'black screen', 'missing', 'won\'t load',
])
...
def chat():
    ...
    if not AI_CHAT_ENABLED:
        return jsonify({'error': 'AI chat is not configured. Set OPENAI_API_KEY in your environment.'}), 503
    user_email = session.get('user_email')
    tier = get_user_tier(user_email) if user_email else 'free'
    if not has_paid_access(tier):
    ...
    if any(kw in msg_lower for kw in _PROBLEM_KEYWORDS) and has_paid_access(tier):
        try:
            ...
            game_obj = next((g for g in SUPPORTED_GAMES if g['id'] == game), None)
            game_name = game_obj['name'] if game_obj else 'Skyrim'
            web_solutions = search_solutions_web(query=message[:80], game_display_name=game_name, max_results=5)
        except Exception as e:
    ...
    try:
        ...
        rec_payload = get_recommendations_for_ai(p, mod_list, game_for_rec, nexus_slug, limit=10, top_picks_per_category=2)
    except Exception:
        pass
...
def scan_game_folder():
    ...
    tier = get_user_tier(user_email) if user_email else 'free'
    if not has_paid_access(tier):
        return jsonify({'error': 'Game folder scan is included with paid tiers (Pro/OpenClaw).'}), 403
    data = request.get_json() or {}
    game = (data.get('game') or DEFAULT_GAME).lower()
    ...
    tree_pruned, key_files_pruned, _ = prune_game_folder_context(tree, key_files, plugins, max_tree_chars=5000, max_file_chars=3000)
    key_block = []
...
# -------------------------------------------------------------------
_DEV_RELEVANT_EXT = ('.psc', '.ini', '.json', '.toml', '.yaml', '.yml', '.txt', '.md')  # text only; .esp/.esm/.esl are binary
...
def api_dev_analyze():
    ...
    if repo_url:
        ...
        if not files_content:
            return jsonify({'error': 'No relevant files found (look for .psc, .esp, .ini, README, etc.)'}), 400
    else:
...
def api_dev_loop_suggest():
    ...
    perf_actions = []
    if crashes > 0:
        perf_actions.append('Prioritize crash triage: reduce script-heavy features and verify dependency versions first.')
    if fps_avg is not None and fps_avg < 55:
        perf_actions.append('Switch to balanced/performance profile and reduce high-VRAM textures before adding new scripted systems.')
    if stutter > 6:
        perf_actions.append('Lower script update frequency and batch expensive events to reduce frametime spikes.')
    if not perf_actions:
        perf_actions.append('Keep current optimization profile and add only one feature change per run for clean A/B verification.')
    ...
    idle_conclusion = (
        ...
        if should_idle else
        'Recommendation: continue iterating with one controlled change per run.'
    )
...
def api_feedback_submit():
    ...
    if kind not in ('bug', 'feature', 'complaint', 'praise', 'question', 'feedback'):
        kind = 'feedback'
    if category not in ('analysis', 'community', 'ui', 'performance', 'billing', 'devtools', 'other', 'general'):
        category = 'general'
    ...
    try:
        ...
        db.commit()
        track_activity('feedback_submit', {'type': kind, 'category': category}, session.get('user_email'))
...
def api_community_report():
    ...
    try:
        ...
        db.commit()
        track_activity('community_report', {'post_id': post_id, 'reply_id': reply_id, 'reason': reason}, session.get('user_email'))
        return jsonify({'success': True})
    except Exception as e:
...
def api_community_health():
    ...
    try:
        ...
        avg_rating = float(avg_rating_row[0]) if avg_rating_row and avg_rating_row[0] is not None else None
        return jsonify({
            'success': True,
            'posts_7d': posts_7d,
...
def openclaw_request_access():
    ...
    if missing:
        return api_error('All safety acknowledgements are required before access can be granted.', 400, missing=missing)
...
def openclaw_verify_grant():
    ...
    _openclaw_log_event(user_email=user_email, event_type='grant_verified', allowed=True, reasons=[])
...
def _openclaw_log_event(user_email: str, event_type: str, operation: str = None, rel_path: str = None, allowed: bool = False, reasons=None):
...
def _issue_openclaw_grant(user_email: str, workspace_rel: str, ttl_seconds: int = 1800) -> str:
    ...
    token = serializer.dumps({'email': user_email.lower(), 'workspace_rel': workspace_rel, 'exp': expires_at, 'rnd': secrets.token_hex(8)})
...
def _validate_openclaw_grant(token: str, user_email: str) -> tuple[bool, str]:
    ...
    if OPENCLAW_REQUIRE_SAME_IP and (row['ip_hash'] or '') and (row['ip_hash'] != _openclaw_ip_hash()):
...
def _openclaw_require_lab_access(data):
    ...
    if not ok:
        _openclaw_log_event(user_email=user_email, event_type='grant_required_failed', allowed=False, reasons=[reason])
...
def openclaw_permissions():
    ...
    if not clean_scopes:
        return api_error('No valid permission scopes requested.', 400, available_scopes=OPENCLAW_PERMISSION_SCOPES)
...
def openclaw_plan_propose():
    ...
    plan_body = build_openclaw_plan(
        game=game,
        objective=(data.get('objective') or 'improve stability and visuals while keeping safety strict'),
    ...
    for idx, a in enumerate(actions):
        ...
        preflight.append({'action_index': idx, 'allowed': allowed, 'reasons': reasons, 'file_action': fa})
    ...
    get_db().commit()
    _openclaw_log_event(user_email=user_email, event_type='plan_proposed', rel_path='.openclaw/plans', allowed=True, reasons=[plan_id])
...
def openclaw_loop_feedback():
    ...
    _openclaw_write_state_json(
        ...
        payload={'feedback': feedback, 'suggestions': suggestions, 'updated_at': datetime.utcnow().isoformat() + 'Z'},
    )
    _openclaw_log_event(user_email=user_email, event_type='loop_feedback', rel_path='.openclaw/loop', allowed=True, reasons=suggestions[:2])
    return jsonify({
        'success': True,
        'game': game,
        'feedback': feedback,
...
def _openclaw_safety_status():
    ...
    checks = [
        ...
        ('max_bytes_guard', OPENCLAW_MAX_BYTES <= 100 * 1024 * 1024, 'Byte guard within conservative cap'),
        ('op_allowlist', len(OPENCLAW_ALLOWED_OPERATIONS) <= 12, 'Operation allowlist is constrained'),
        ('denylist_has_system_markers', any(x in OPENCLAW_DENY_SEGMENTS for x in ('system32', 'bios', 'efi', 'registry')), 'System-critical path denylist present'),
    ]
    ...
    return jsonify({
        ...
        'companion': {
            ...
            'required_permissions': [
                {'scope': 'launch_game', 'prompt': 'Allow OpenClaw to launch the game through a wrapper intent?'},
                {'scope': 'read_game_logs', 'prompt': 'Allow read-only parsing of runtime/game logs?'},
                {'scope': 'read_performance_metrics', 'prompt': 'Allow collection of FPS/frametime summaries?'},
                {'scope': 'controller_signal', 'prompt': 'Allow aggregated controller usage signal?'},
                {'scope': 'input_signal_aggregate', 'prompt': 'Allow aggregated input pattern signal (no raw key logging)?'},
                {'scope': 'internet_research', 'prompt': 'Allow trusted web lookup for compatibility/performance ideas?'},
                {'scope': 'write_sandbox_files', 'prompt': 'Allow writing plans/checkpoints only inside OpenClaw sandbox workspace?'},
            ],
            'blocked_hard': [
...
def api_profile_links_save():
    ...
    if links['nexus_profile_url'] and not links['nexus_profile_url'].startswith(('https://www.nexusmods.com/', 'https://next.nexusmods.com/')):
...
def api_profile_dashboard():
    ...
    try:
        ...
        suggestions = []
        if posts_count == 0:
            suggestions.append('Introduce yourself in Community with your game + mod manager + plugin count.')
        if replies_count < 2:
            suggestions.append('Reply to a help thread to build trust and discover edge-case fixes.')
        if feedback_count == 0:
            suggestions.append('Use the feedback button after your next run so Samson can tune suggestions better.')
        if avg_helpfulness is None or (avg_helpfulness and avg_helpfulness < 3.5):
            suggestions.append('Run Analyze after Build a List and follow red errors first, then warnings.')
        if not top_tags:
            suggestions.append('Pick a community tag (tip/help/dev) on your next post so others can find you faster.')
        suggestions = suggestions[:4]

        track_activity('profile_dashboard_view', {'posts': posts_count, 'replies': replies_count}, user_email)
        return jsonify({
            'success': True,
            'stats': {
                ...
                'avg_helpfulness': round(float(avg_helpfulness), 2) if avg_helpfulness is not None else None,
            },
            'top_tags': top_tags,
...
def api_specs_save():
    ...
    return jsonify({'success': True, 'specs': specs or (get_user_specs(user_email) if user_email else {})})
...
def logout():
    ...
    resp.set_cookie(SESSION_COOKIE_NAME, '', max_age=0, httponly=True, secure=app.config['SESSION_COOKIE_SECURE'], samesite=app.config['SESSION_COOKIE_SAMESITE'])
    return resp
...
def information_map():
    ...
    return jsonify({
        ...
        'inputs': {
            'user_inputs': ['mod_list', 'build_preferences', 'specs', 'community_posts', 'dev_files', 'feedback'],
            'runtime_signals': ['activity_events', 'satisfaction_scores', 'openclaw_loop_feedback'],
            'external_sources': ['LOOT masterlist', 'optional web search fallback', 'public GitHub repo content (dev mode)'],
        },
        'processing': {
            'parsing': ['parse_mod_list_text', 'modlist_normalize'],
            'ranking': ['BM25 search engine', 'recommendation heuristics', 'system impact scoring'],
            'ai_layers': ['analysis assistant', 'dev assistant', 'samson loop suggestions', 'openclaw plan engine'],
            'safety_layers': ['rate limits', 'tier gates', 'openclaw grant token', 'immutable guard-check policy'],
        },
        'stores': {
            'users_db_tables': [
                'users', 'user_sessions', 'api_keys', 'user_specs', 'user_links',
                'community_posts', 'community_replies', 'community_votes', 'community_reports',
                'user_feedback', 'user_activity', 'satisfaction_surveys',
                'openclaw_grants', 'openclaw_events', 'openclaw_permissions',
                'openclaw_plan_runs', 'openclaw_feedback',
            ],
...
def health():
    ...
    return jsonify({
        ...
        'stripe_openclaw_configured': bool(stripe_openclaw_price_id) if PAYMENTS_ENABLED else False,
