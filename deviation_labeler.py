...
def analyze_source_deviations(source: KnowledgeSource) -> Tuple[List[str], str]:
    ...
    experimental_keywords = [
        "experimental", "beta", "wip", "work in progress",
        "unstable", "use at own risk", "not recommended",
...
def check_conflicting_approaches(source: KnowledgeSource) -> bool:
    ...
    try:
        ...
        similar = session.query(KnowledgeSource).filter(
...
def extract_methods(text: str) -> set:
    ...
    method_keywords = [
        "install", "download", "enable", "disable",
        "patch", "merge", "load order",
        "skse", "enb", "replacer", "modular",
        "manual", "automatic", "mo2", "vortex"
    ]
...
def has_version_issues(source: KnowledgeSource) -> bool:
    ...
    version_keywords = [
        "only works with", "requires version",
        "broken in", "compatible with",
        "version specific", "ae only", "se only",
        "1.6.1170", "1.5.97", "next-gen"
    ]
