...
def directory():
    ...
    config = get_config_loader()
    categories = config._load_yaml(config.config_dir / "business_categories.yaml").get(
        "approved", []
    )
...
def join():
    ...
    if request.method == "POST":
        ...
        if business_id:
            flash(
                "Your business has been submitted for review. We'll approve it within 7 days.",
                "success",
            )
...
def dashboard():
    ...
    for biz in businesses:
        ...
        business_metrics.append({"business": biz, "trust": trust})

    return render_template("business/dashboard.html", businesses=business_metrics, metrics={})
