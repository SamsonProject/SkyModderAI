...
_CPU_DATABASE = {
    ...
    "ryzen 9 7950": (6, 16, 92),
    "ryzen 9 7900": (5, 12, 85),
    "ryzen 7 7800": (5, 8, 82),
    "ryzen 7 7700": (4, 8, 75),
    "ryzen 5 7600": (4, 6, 68),
    "ryzen 9 5950": (5, 16, 88),
    "ryzen 9 5900": (5, 12, 82),
    "ryzen 7 5800": (4, 8, 75),
    "ryzen 5 5600": (3, 6, 60),
    "ryzen 5 5500": (3, 6, 55),
    "ryzen 3 5300": (2, 4, 42),
    # AMD Ryzen 3000/2000-series
    "ryzen 9 3950": (4, 16, 80),
    "ryzen 9 3900": (4, 12, 75),
    "ryzen 7 3800": (4, 8, 70),
    "ryzen 7 3700": (3, 8, 65),
    "ryzen 5 3600": (3, 6, 55),
    "ryzen 5 2600": (2, 6, 48),
...
_HEAVY_PATTERNS = [
    ...
    (
        r"\b(high\s*poly|highpoly)\b",
        "High-poly meshes",
        "medium",
        {"fps_impact": -8, "vram_gb": 0.5},
    ),
    ...
    (
        r"\b(legacy\s*of\s*the\s*dragonborn|lotd)\b",
        "LotD",
        "high",
        {"fps_impact": -12, "vram_gb": 1.0},
    ),
    (r"\b(beyond\s*skyrim|bruma)\b", "Beyond Skyrim", "high", {"fps_impact": -10, "vram_gb": 0.8}),
    (
        r"\b(flora\s*overhaul|sfo|skyrim\s*flora)\b",
        "Flora overhaul",
        "medium",
        {"fps_impact": -8, "vram_gb": 0.5},
    ),
    (
        r"\b(grass\s*mod|grass\s*overhaul)\b",
        "Grass mod",
        "medium",
        {"fps_impact": -10, "vram_gb": 0.5},
    ),
    (
        r"\b(weather\s*mod|obsidian|cathedral|rudy)\b",
        "Weather/lighting",
        "medium",
        {"fps_impact": -8, "vram_gb": 0.5},
    ),
    (
        r"\b(immersive\s*citizens|ai\s*overhaul)\b",
        "AI/NPC overhaul",
        "medium",
        {"fps_impact": -6, "vram_gb": 0.3},
    ),
    (
        r"\b(water\s*mod|realistic\s*water)\b",
        "Water mod",
        "low",
        {"fps_impact": -4, "vram_gb": 0.3},
    ),
    (
        r"\b(animation\s*mod|nemesis|fnis)\b",
        "Animation framework",
        "medium",
        {"fps_impact": -5, "vram_gb": 0.2},
    ),
...
def _calculate_bottleneck(gpu_score: int, cpu_score: int, game: str) -> dict:
    ...
    if not gpu_score or not cpu_score:
        return {
            "type": "unknown",
            "severity": "unknown",
            "explanation": "Insufficient hardware data",
        }
...
def _estimate_fps(
    ...
    res_mult = {"1080p": 1.0, "1440p": 0.75, "4k": 0.55, "ultrawide": 0.7}.get(
        resolution.lower(), 1.0
    )

    estimated_fps = max(15, base_fps * hardware_factor * res_mult + total_fps_impact)
    confidence = (
        "high" if gpu_score > 0 and cpu_score > 0 else "medium" if gpu_score or cpu_score else "low"
    )

    return {
        "estimated": round(estimated_fps),
        "min": round(max(10, estimated_fps - 10)),
        "max": round(min(estimated_fps + 15, base_fps * hardware_factor)),
        ...
        suggestions.append(
            {
                "priority": "medium",
                "category": "grass",
                "title": "Grass & Flora Optimization",
                "description": "Grass mods impact both CPU and GPU.",
                "actions": [
                    "Reduce grass density in INI",
                    "Use optimized grass textures",
        ...
        suggestions.append(
            {
                ...
                "actions": [
                    "Move game to SSD if possible",
                    "Disable disk-heavy mods",
...
def get_system_impact(
    ...
    estimated_vram = min(
        24.0,
        (base_vram + (enabled_count * 0.02) + heavy_bonus) * _GAME_VRAM_MULTIPLIERS.get(game, 1.0),
    )
    ...
    fps_estimate = _estimate_fps(
        game_baseline.get("base_fps", 60),
        heavy_mods,
        gpu_score,
        cpu_score,
        specs.get("resolution", "1080p"),
    )
    ...
    if specs.get("vram_gb") or specs.get("vram"):
        try:
            vram_spec = int(
                str(specs.get("vram_gb") or specs.get("vram")).replace("gb", "").strip()
            )
        except (ValueError, TypeError):
            pass
    ...
    heavy_mods_dict = [
        {
            ...
            "category": m.category,
            "impact": m.impact,
            "fps_impact": m.fps_impact,
            "vram_gb": m.vram_gb,
        }
        for m in heavy_mods[:20]
    ]
    ...
    return {
        ...
        "heavy_mods": heavy_mods_dict,
        "impact_ranking": heavy_mods_dict,
...
def format_system_impact_for_ai(si: dict) -> str:
    ...
    if bottleneck.get("type") != "unknown":
        lines.append(
            f"Bottleneck: {bottleneck.get('type', '?').upper()} ({bottleneck.get('severity', '?')})"
        )
...
def format_system_impact_report(si: dict) -> str:
    ...
    if si.get("has_specs"):
        ...
        if bottleneck.get("type"):
            lines.append(
                f"  Bottleneck: {bottleneck.get('type', '?').upper()} - {bottleneck.get('explanation', '')}"
            )
        lines.append("")
    ...
    if heavy:
        lines.extend(
            ["MODS WITH HIGH PERFORMANCE IMPACT"]
            + [
