...
from models import KnowledgeSource, SourceCredibility, TrashBinItem
...
def run_information_compaction() -> Dict[str, Any]:
    ...
    try:
        ...
        sources = session.query(KnowledgeSource).filter(
            KnowledgeSource.status == "active",
            KnowledgeSource.content_hash.isnot(None)
        ).all()
...
def run_trash_audit() -> Dict[str, Any]:
    ...
    try:
        ...
        items = session.query(TrashBinItem).filter(
            TrashBinItem.created_at < cutoff,
        ...
        old_items = session.query(TrashBinItem).filter(
            TrashBinItem.created_at < old_cutoff
        ).all()
...
def run_category_discovery() -> Dict[str, Any]:
    ...
    try:
        ...
        sources = session.query(KnowledgeSource).filter(
            KnowledgeSource.status == "active",
            KnowledgeSource.category.isnot(None)
        ).all()
        ...
        for category, cat_sources in category_sources.items():
            ...
            for tag, count in tag_counts.items():
                if count >= len(cat_sources) * 0.3:  # 30%+ have this tag
                    ...
                    subcat_exists = any(
                        s.subcategory == tag for s in cat_sources
                    )
