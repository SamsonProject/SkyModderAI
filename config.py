...
class Config:
    ...
    SESSION_COOKIE_SECURE = (
        os.getenv("SESSION_COOKIE_SECURE", str(FLASK_ENV == "production")).lower() == "true"
    )
    ...
    def validate_config(cls):
        ...
        if cls.FLASK_ENV == "production":
            ...
            if not all(
                [cls.STRIPE_PUBLISHABLE_KEY, cls.STRIPE_SECRET_KEY, cls.STRIPE_PRO_PRICE_ID]
            ):
                logger.warning(
            ...
            if not cls.BASE_URL or cls.BASE_URL == "http://localhost:5000":
                raise ValueError(
                    "BASE_URL must be set to your production domain in production environment."
                )
