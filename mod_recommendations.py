...
def get_recommendations_for_ai(
    ...
    """
    recs = get_recommendations(parser, mod_names, game, nexus_slug, limit=limit, include_category=True)
    # Bucket by category
...
def _curated_fallback(parser, game: str, nexus_slug: str, category: str, limit: int) -> List[Dict[str, Any]]:
    """Fill top_picks with curated search results when we have no matches in that category."""
    from search_engine import get_search_engine
    terms = _CURATED_TOP_PICKS.get(game, _CURATED_TOP_PICKS.get('skyrimse', {})).get(category, ['patch'])
...
def _norm(s: str) -> str:
    return (s or '').lower().strip().replace('.esp', '').replace('.esm', '').replace('.esl', '').strip()
