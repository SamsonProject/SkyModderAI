...
def benchmark_presentation(runner: Benchmark):
    ...
    from presentation_service import format_as_latex, format_as_html, _format_as_markdown, create_guide_content
    ...
    content = create_guide_content(
        ...
        sources=[{"title": f"Source {i}", "url": "https://example.com", "credibility_score": 0.9} for i in range(5)]
    )
    
    # LaTeX formatting
    runner.run(
        format_as_latex, content,
        iterations=10,
